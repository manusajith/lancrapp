RssFeed.publish('jobs', function(query) {
  ...
  self.setValue('title', self.cdata('Lancr - Recent Jobs'));
  self.setValue('description', self.cdata('This is a feed of recent jobs posted to Lancr.'));
...
var profileRss = function(query) {
  ...
  self.setValue('title', self.cdata('Lancr - Recent Profiles'));
  self.setValue('description', self.cdata('This is a feed of recent profiles listed on Lancr.'));
  self.setValue('link', Meteor.absoluteUrl());
  self.setValue('lastBuildDate', lastBuildDate);
  self.setValue('pubDate', pubDate);
  self.setValue('ttl', 1);
